<template>
    <table v-if="tableHead.length > 0" class="responsive-table w-full" :class="[{'shadow-md' : (shadow)}, bg]">
        <thead class="hidden" :class="(!hideHead) ? 'sm:table-header-group' : ''">
        <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
            <th v-if="selectable" class="py-3 px-6 w-6 text-center">Select</th>
            <template v-for="(tableHeaderItem,index) in tableHead" :key="index">
        ...
        <tr
            class="block text-right m-4 sm:table-row hover:bg-gray-100"
            v-for="(rowData, rowIndex) in tableData"
            :key="rowIndex"
            :class="{'border-b border-gray-200' : (tableData.length > rowIndex)}"
        >
            <td v-if="selectable" class="block sm:table-cell py-3 px-6" data-label="Select">
                <input
                    @change="selectItem($event, rowData)"
                    type="checkbox"
                    class="appearance-none checked:bg-blue-600 checked:border-transparent"
                />
            </td>
            <td
                class="block sm:table-cell py-3 px-6"
                v-for="(tableHeaderItem,index) in tableHead"
                :key="index"
                :data-label="tableHeaderItem.label"
                :class="`sm:${tableHeaderItem.align}`"
            >
